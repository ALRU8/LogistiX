...
Secret_token = "dsadasdadasd2q332q"
...
def emergency_stop():
    try:
        ...
        controller.connect("COM7", 115200)
...
def start_program():
    ...
    while True:
        ...
        time.sleep(0.1)
    time.sleep(2)
    controller.z_move_pos(-50, 800)
    coords['z'] -= 50
    while True:
        controller.position_timer()
        current_z = controller.wp_axe_z
        print(current_z)
        if abs(current_z - (-59)) < 1:
            break
        time.sleep(0.1)
    send_servo_command(1, 20, 2, angle_2)
    angle_1 = 30
    ...
    while True:
        ...
        time.sleep(0.1)
    time.sleep(1)
    controller.z_move_pos(-40, 800)
    coords['z'] -= 40
    while True:
        controller.position_timer()
        current_z = controller.wp_axe_z
        print(current_z)
        if abs(current_z - (-40)) < 1:
            break
        time.sleep(0.1)
    time.sleep(1)
